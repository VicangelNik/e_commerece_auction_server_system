...
import com.vicangel.e_commerce_auction_server_system.endpoint.rest.controllers.mappers.AuctionEndpointMapperImpl;
...
final class AuctionEndpointMapperTest {
  ...
  void mapRequestToModel() {
    ...
    final Auction expected = Auction.builder().ends(ends).firstBid(7.00F).numberOfBids(3).sellerId(1L).build();

    final Auction actual = mapper.mapRequestToModel(SaveAuctionRequest.builder().ends(ends)
                                                      .firstBid(7.00F)
                                                      .numberOfBids(3)
                                                      .sellerId(1L)
                                                      .build());
